...
public class AddDamageMagic : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (PlayerPrefs.HasKey("BaseCoinMagic"))
        {
            baseCoin = PlayerPrefs.GetInt("BaseCoinMagic");
        }
    ...
    public void Upgrade()
    {
        if (coinManager != null && baseCoin <= coinManager.CoinCount)
        {
            ...
            PlayerPrefs.SetInt("BaseCoinMagic", baseCoin);
            PlayerPrefs.Save();
